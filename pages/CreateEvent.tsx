...
const CreateEvent: React.FC<CreateEventProps> = ({ navigateTo, onAddEvent, profile }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        // Persiste localmente para o sucesso da publicação
  ...
  return (
    <div className="flex flex-col w-full pb-32 min-h-screen bg-slate-50 dark:bg-zinc-950 animate-in fade-in duration-500">
      {errorModal.show && (
      ...
      <header className="p-8 border-b border-zinc-100 dark:border-white/5 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-3xl sticky top-0 z-50">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <h1 className="text-[12px] font-[900] uppercase tracking-[0.3em] text-zinc-900 dark:text-white">Publicar Evento</h1>
          ...
          <div className="space-y-10 animate-in slide-in-from-bottom-6">
            ...
            <div className="space-y-8">
              <div className="space-y-2.5">
                ...
                <input 
                  type="text" 
                  placeholder="EX: I CONGRESSO DE TECNOLOGIA IFAL" 
          ...
          <div className="space-y-10 animate-in slide-in-from-right-6">
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2.5">
                  <label className="text-[11px] font-black text-zinc-500 uppercase">Data Institucional</label>
                  <input type="text" placeholder="EX: 15 DE NOVEMBRO" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="h-20 px-6 w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 rounded-3xl font-black text-sm dark:text-white outline-none" />
                </div>
                <div className="space-y-2.5">
                  <label className="text-[11px] font-black text-zinc-500 uppercase">Localização / Sala</label>
              ...
              <div className="space-y-2.5">
                <label className="text-[11px] font-black text-zinc-500 uppercase">Tipo de Evento</label>
          ...
          <div className="space-y-10 animate-in zoom-in-95 duration-500">
            ...
            <div className="space-y-8">
              <div 
                ...
                className="aspect-video w-full rounded-[2.5rem] bg-slate-100 dark:bg-zinc-900 border-2 border-dashed border-zinc-200 dark:border-white/10 overflow-hidden shadow-inner relative group cursor-pointer transition-all hover:border-primary/50"
              >
              ...
              <div className="space-y-2.5">
                ...
                <textarea 
                  rows={6}
                  placeholder="Descreva o impacto acadêmico do seu evento..."
                  value={formData.description}
                  onChange={e => setFormData({...formData, description: e.target.value})}
                  className="w-full p-8 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/5 rounded-[2.5rem] font-bold text-sm dark:text-white outline-none resize-none focus:ring-4 focus:ring-primary/10 transition-all"
                />
              </div>
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto w-full flex gap-5">
          ...
          >
            {isPublishing ? (
              ...
              <>{step === 3 ? 'Publicar Agora' : 'Próxima Etapa'} <span className="material-symbols-outlined text-xl">{step === 3 ? 'rocket_launch' : 'arrow_forward'}</span></>
            )}
          </button>
        </div>
      </footer>
