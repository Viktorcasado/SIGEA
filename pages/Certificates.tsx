...
const Certificates: React.FC<CertificatesProps> = ({ navigateTo, eventId, user, role = UserRole.PARTICIPANT }) => {
  ...
  useEffect(() => {
    if (eventId && isOrganizer) fetchTemplates();
  }, [eventId, activeTab]);
  ...
  const handleAddTemplate = async () => {
    const title = prompt("Título do Certificado (ex: Certificado de Palestrante):");
    if (!title) return;
    
    const { error } = await supabase.from('certificate_templates').insert([{
      event_id: eventId,
      title: title,
      attribution: 'Somente os credenciados',
      status: 'Publicado'
    }]);
  ...
  const renderOrganizerView = () => (
    <div className="flex flex-col w-full h-full bg-white dark:bg-zinc-950 animate-in fade-in duration-500">
      <header className="p-8 border-b border-slate-200 dark:border-white/5">
        <div className="flex items-center gap-4 mb-8">
           ...
           <h1 className="text-2xl font-light text-slate-800 dark:text-white">Certificado</h1>
        </div>
          ...
          <>
              ...
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <button 
                    ...
                    className="h-10 px-6 bg-primary text-white rounded-md text-sm font-medium flex items-center gap-2 hover:bg-secondary transition-all shadow-lg shadow-primary/20"
                  >
                ...
                <div className="overflow-hidden border border-slate-200 dark:border-white/5 rounded-lg shadow-sm">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                      {templates.length > 0 ? templates.map(cert => (
                        <tr key={cert.id} className="text-sm text-slate-600 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-white/5">
                          ...
                          <td className="px-6 py-5">
                            <span className="px-2 py-1 bg-green-100 text-green-700 dark:bg-green-500/10 dark:text-green-400 rounded-md text-[10px] font-bold uppercase tracking-tighter">
                              {cert.status}
                            </span>
                          </td>
                        </tr>
                      )) : (
                        <tr><td colSpan={4} className="px-6 py-10 text-center text-slate-400 text-xs uppercase font-bold tracking-widest">Nenhum modelo criado</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
            ...
            {activeTab === 'config' && (
              <div className="max-w-4xl space-y-12 animate-in fade-in duration-300">
                ...
                <div className="p-8 border border-slate-200 dark:border-white/5 rounded-lg space-y-4 shadow-sm">
                  <h3 className="font-bold text-slate-800 dark:text-white">Comunicação</h3>
                  <p className="text-sm text-slate-500">Edite o conteúdo do email que o participante vai receber.</p>
    ...
    <div className="flex flex-col w-full min-h-screen bg-white dark:bg-zinc-950 animate-in fade-in duration-500">
      {isOrganizer ? renderOrganizerView() : (
        <>
          ...
          <main className="p-6">
            <div className="py-24 text-center bg-slate-50 dark:bg-zinc-900/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-white/10">
               <span className="material-symbols-outlined text-4xl text-slate-300 mb-4">workspace_premium</span>
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-8">Seus certificados aparecerão aqui após a conclusão dos eventos.</p>
            </div>
          </main>
        </>
      )}
